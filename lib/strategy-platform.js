const STRATEGY_API_ENDPOINT = "/api/strategy";

function isDevelopmentMode() {
  return process.env.NODE_ENV === "development";
}

function resolveLocalSourceLabel() {
  return isDevelopmentMode() ? "Development Mock" : "Fallback Market Model";
}

function resolveLocalConnectionStatus() {
...
function buildFallbackSnapshot() {
  return {
    ...
    source: resolveLocalSourceLabel(),
    connectionStatus: resolveLocalConnectionStatus(),
    dataSource: {
      source: resolveLocalSourceLabel(),
      connectionStatus: resolveLocalConnectionStatus(),
      mode: isDevelopmentMode() ? "development" : "fallback",
    },
    updatedAt: new Date().toISOString(),
    integration: buildServiceStatus(resolveLocalSourceLabel(), resolveLocalConnectionStatus()),
    narratives: {
      dominantNarrative: {
...
function buildServiceStatus(provider = "CoinMarketCap Data", status = "CoinMarketCap Connected") {
  return {
    provider,
    status,
    notes: [
      "CoinMarketCap categories and market metrics are normalized into the dashboard snapshot.",
      "Keep the dashboard panels unchanged; only the data source adapter should move.",
      "Source labels switch automatically between live, fallback, and development modes.",
    ],
  };
}
...
export async function loadStrategyPlatformSnapshot({ signal } = {}) {
  try {
    const response = await fetchJson(STRATEGY_API_ENDPOINT, signal);
    const payload = response.data ?? response;

    return {
      ...payload,
      platform: payload.platform ?? "CMC NarrativeX Agent",
      source: payload.source ?? "CoinMarketCap Data",
      connectionStatus: payload.connectionStatus ?? "CoinMarketCap Connected",
      dataSource:
        payload.dataSource ?? {
          source: payload.source ?? "CoinMarketCap Data",
          connectionStatus: payload.connectionStatus ?? "CoinMarketCap Connected",
      ...
      integration: buildServiceStatus(),
    };
  } catch {

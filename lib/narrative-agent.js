...
function scoreMomentum(candidate) {
  return (
    Number(candidate?.avgPriceChange) * 1.35 +
    Number(candidate?.volumeChange) * 1.1 +
    Number(candidate?.marketCapChange) * 0.85
  );
}
...
function scoreCandidatePriority(candidate, { profile, holding, regime, riskScore }) {
  ...
  const signalGap = Math.max(0, candidate.score - (candidate.peerScore ?? 0));
  const volatility = Math.abs(Number(candidate.avgPriceChange) - Number(candidate.volumeChange) / 3);
  const regimeTilt =
    regime.name === "Bull"
      ? momentum * 0.35 + candidate.score * 0.08
      : regime.name === "Euphoria"
        ? momentum * 0.42 - Math.max(0, lifecycle * -0.4)
        : regime.name === "Sideways"
          ? candidate.marketCapChange * 0.95 + lifecycle * 0.45
          : regime.name === "Bear"
            ? candidate.marketCapChange * 1.05 + lifecycle * 0.55 - momentum * 0.08
            : candidate.marketCapChange * 1.15 + lifecycle * 0.7 - momentum * 0.12;

  const profileTilt =
    profile.name === "Conservative"
      ? candidate.marketCapChange * profile.stabilityBias + lifecycle * 0.4 - volatility * 0.9
      : profile.name === "Aggressive"
        ? momentum * profile.momentumBias + signalGap * 0.65 - Math.max(0, 5 - candidate.score) * 0.1
        : momentum * 0.45 + candidate.marketCapChange * 0.55 + lifecycle * 0.25;

  const holdingTilt =
    holding.name === "1 Week"
      ? momentum * holding.momentumBias + signalGap * 0.5 - volatility * 0.6
      : holding.name === "2 Weeks"
        ? momentum * holding.momentumBias + candidate.volumeChange * 0.4
        : holding.name === "1 Month"
          ? candidate.marketCapChange * 0.7 + lifecycle * 0.45 + momentum * 0.2
          : candidate.marketCapChange * 1.1 + lifecycle * 0.7 - Math.max(0, momentum - 15) * 0.08;
  ...
  const ranked = candidates
    .map((candidate, index, list) => {
      ...
      return {
        ...candidate,
        priority,
        momentumScore: scoreMomentum(candidate),
      };
    })
...
function buildAllocationWeights(rankedCandidates, snapshot, riskProfile, holdingPeriod) {
  ...
  const dominantSignal = dominant ? dominant.priority : dominant?.score ?? 0;
  const secondSignal = second ? second.priority : dominantSignal * 0.82;
  const thirdSignal = third ? third.priority : secondSignal * 0.8;
  const signalGap = dominant && second ? clamp(dominant.priority - second.priority, 0, 40) : 0;
  const momentumBoost = dominant ? dominant.momentumScore / 18 : 0;
  ...
  const secondRaw = Math.max(
    ...
    secondSignal * (0.82 + profile.concentrationBias / 2 + holding.entryBias / 2 + regime.exposureBias / 2) +
      (second ? second.momentumScore : 0) * 0.12 +
  ...
  const thirdRaw = Math.max(
    ...
    thirdSignal * (0.66 + profile.concentrationBias / 4 + holding.exitBias / 2 + regime.exposureBias / 3) +
      (third ? third.marketCapChange : 0) * 0.9 +
      (snapshot?.narratives?.narrativeStrength?.score ? snapshot.narratives.narrativeStrength.score / 15 : 0),
  );
  ...
  const exposureBase = clamp(
    0.54 +
      ...
      regime.exposureBias -
      riskPenalty +
      (dominant ? dominant.momentumScore / 300 : 0),
    0.24,
    0.94,
  );
...
function buildWatchlist(rankedCandidates, riskProfile, holdingPeriod) {
  ...
  return rankedCandidates.slice(0, 5).map((candidate, index) => {
    const momentum = candidate.momentumScore;
    const rank = index + 1;
    const tone =
      rank === 1
        ? "pulse"
...
function buildConfidenceScore({
  ...
  const regime = getRegimeMeta(snapshot?.regime?.active ?? "Sideways");
  const riskScore = clamp(Number(snapshot?.risk?.score) || 50, 0, 100);
  const narrativeStrength = clamp(Number(snapshot?.narratives?.narrativeStrength?.score) || rankedCandidates[0]?.score || 50, 0, 100);
  const rotationScore = clamp(Number(snapshot?.narratives?.narrativeRotationScore?.score) || rankedCandidates[0]?.score || 50, 0, 100);
  ...
  const scoreGap = Math.max(0, topCandidate.priority - (secondCandidate?.priority ?? topCandidate.priority));
  const momentum = topCandidate.momentumScore;

  const base = (topCandidate.score * 0.34 + narrativeStrength * 0.23 + rotationScore * 0.18) / 0.75;
  const profileAdjustment =
    profile.name === "Conservative"
      ? 7 - riskScore * 0.12 - Math.max(0, momentum - 12) * 0.2
      : profile.name === "Moderate"
        ? 2 + (riskScore < 55 ? 2 : -2)
        : 8 + Math.max(0, momentum - 4) * 0.35 - Math.max(0, 45 - riskScore) * 0.05;
  const holdingAdjustment =
    holding.name === "1 Week"
      ? Math.max(0, momentum) * 0.18 - 2
      : holding.name === "2 Weeks"
        ? Math.max(0, momentum) * 0.12
        : holding.name === "1 Month"
          ? (topCandidate.marketCapChange + topCandidate.score / 10) * 0.08 + 2
          : (topCandidate.marketCapChange + topCandidate.score / 12) * 0.12 + 4;
  const regimeAdjustment = regime.confidenceBias;
  const gapAdjustment = scoreGap * 0.45;
  const riskAdjustment = (100 - riskScore) * 0.08;

  return clamp(
    Math.round(base + profileAdjustment + holdingAdjustment + regimeAdjustment + gapAdjustment + riskAdjustment),
    0,
    100,
  );
}
...
function buildRiskNotes({
  ...
  const secondCandidate = rankedCandidates[1];
  const topGap = Math.max(0, dominantCandidate?.priority - (secondCandidate?.priority ?? dominantCandidate?.priority));
...
export function buildNarrativeRotationAgent(snapshot, inputs) {
  ...
  const reasoningSummary = [
    ...
    `That ranking is shaped by current narrative strength, category momentum, market regime, risk score, and the ${riskProfile.toLowerCase()} / ${holdingPeriod.toLowerCase()} setup.`,
  ].join(" ");
